...
#define ActionPaste 1
#define ActionCopy 2
#define ActionCut 3
#define ActionUndo 4            // It's written as defines here because I want to keep Resonate changes collected.
#define ActionRedo 5
#define ActionSelectAll 6
#define ActionCancel 7
#define GetClipboardAction() ((int)ImGui::GetIO().ClipboardUserData)
        const ImGuiInputFlags f_repeat = ImGuiInputFlags_Repeat;
        const bool is_cut   = (GetClipboardAction() == ActionCut || Shortcut(ImGuiMod_Shortcut | ImGuiKey_X, f_repeat, id) || Shortcut(ImGuiMod_Shift | ImGuiKey_Delete, f_repeat, id)) && !is_readonly && !is_password && (!is_multiline || state->HasSelection());
        const bool is_copy  = (GetClipboardAction() == ActionCopy || Shortcut(ImGuiMod_Shortcut | ImGuiKey_C, 0, id) || Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, 0, id))  && !is_password && (!is_multiline || state->HasSelection());
        const bool is_paste = (GetClipboardAction() == ActionPaste || Shortcut(ImGuiMod_Shortcut | ImGuiKey_V, f_repeat, id) || Shortcut(ImGuiMod_Shift | ImGuiKey_Insert, f_repeat, id)) && !is_readonly;
        const bool is_undo  = (GetClipboardAction() == ActionUndo || Shortcut(ImGuiMod_Shortcut | ImGuiKey_Z, f_repeat, id)) && !is_readonly && is_undoable;
        const bool is_redo =  (GetClipboardAction() == ActionRedo || Shortcut(ImGuiMod_Shortcut | ImGuiKey_Y, f_repeat, id) || (is_osx && Shortcut(ImGuiMod_Shortcut | ImGuiMod_Shift | ImGuiKey_Z, f_repeat, id))) && !is_readonly && is_undoable;
        const bool is_select_all = GetClipboardAction() == ActionSelectAll || Shortcut(ImGuiMod_Shortcut | ImGuiKey_A, 0, id);

        // We allow validate/cancel with Nav source (gamepad) to makes it easier to undo an accidental NavInput press with no keyboard wired, but otherwise it isn't very useful.
        const bool nav_gamepad_active = (io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad) != 0 && (io.BackendFlags & ImGuiBackendFlags_HasGamepad) != 0;
        const bool is_enter_pressed = IsKeyPressed(ImGuiKey_Enter, true) || IsKeyPressed(ImGuiKey_KeypadEnter, true);
        const bool is_gamepad_validate = nav_gamepad_active && (IsKeyPressed(ImGuiKey_NavGamepadActivate, false) || IsKeyPressed(ImGuiKey_NavGamepadInput, false));
        const bool is_cancel = GetClipboardAction() == ActionCancel || Shortcut(ImGuiKey_Escape, f_repeat, id) || (nav_gamepad_active && Shortcut(ImGuiKey_NavGamepadCancel, f_repeat, id));
#undef GetClipboardAction()
