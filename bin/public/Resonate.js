...
var ASM_CONSTS = {
  6033910: () => { return Date.now(); },  
 6033931: () => { return Date.now(); },  
 6033952: () => { return Date.now(); },  
 6033973: () => { location.reload() },  
 6033991: () => { location.reload(); },  
 6034010: ($0) => { init_gapi_with_key($0); },  
 6034036: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 6034159: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 6034261: ($0, $1, $2) => { Module.show_loading_screen($0, $1, $2); },  
 6034301: () => { Module.hide_loading_screen(); },  
 6034331: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 6034394: ($0, $1) => { global_audio_element.addEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 6034487: ($0, $1) => { global_audio_element.removeEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 6034583: () => { return global_audio_element.paused ? 1 : 0; },  
 6034627: () => { return global_audio_element.paused ? 1 : 0; },  
 6034671: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 6034721: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 6034977: ($0) => { let error_type = $0; let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 6035267: () => { audio_element_pause(); },  
 6035290: () => { audio_element_play(); },  
 6035312: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
function open_resonate_issues() { window.open('https://github.com/98ahni/Resonate-App/issues/new/choose', '_blank'); }
...
function download_project_zip(zip_name,path_list) { const zipName = Emval.toValue(zip_name); const pathList = Emval.toValue(path_list); var fileBlobs = []; for(let i = 0; i < pathList.length && DEBUG; i++){ console.log(pathList[i]); } for(let i = 0; i < pathList.length; i++){ const docData = FS.readFile(pathList[i]); const docBlob = new File([new Blob([docData.buffer], {type: 'application/octet-binary'})], pathList[i].split('/').pop(), {type: 'application/octet-binary'}); if(DEBUG){console.log(docBlob.name);} fileBlobs.push(docBlob); } return Emval.toHandle(new Promise((resolve)=>{ const zip = new Zip(zipName); zip.files2zip(fileBlobs).then(()=>{ zip.makeZip(); resolve(); }); })); }
...
function create_picker(APIKey,mime_types,file_callback_name,done_callback_name,cancel_callback_name,excluded_extensions) { const view = new google.picker.DocsView() .setIncludeFolders(true) .setMimeTypes(Emval.toValue(mime_types)) .setSelectFolderEnabled(true); const callback_func = Module[Emval.toValue(file_callback_name)]; const done_callback_func = Module[Emval.toValue(done_callback_name)]; const cancel_callback_func = Module[Emval.toValue(cancel_callback_name)]; const ex_extensions = Emval.toValue(excluded_extensions).split(/, \.|, |,\.|,| \.| |\./); const picker = new google.picker.PickerBuilder() .setDeveloperKey(Emval.toValue(APIKey)) .setAppId(824603127976) .setOAuthToken(gapi.client.getToken().access_token) .setTitle('Choose a folder') .addView(view) .addView(new google.picker.DocsUploadView()) .setCallback(async(data) => {if (data.action === google.picker.Action.CANCEL){cancel_callback_func();} if (data.action === google.picker.Action.PICKED){ const documents = data[google.picker.Response.DOCUMENTS]; if(!FS.analyzePath("/GoogleDrive").exists){ FS.mkdir("/GoogleDrive"); } let loadPromises = []; const encoder = new TextEncoder(); for(const document of documents){ const fileId = document[google.picker.Document.ID]; if(DEBUG){console.log(fileId);} const files = []; const res = await gapi.client.drive.files.list({ q: "'" + fileId + "' in parents", fields: 'nextPageToken, files(id, name, fileExtension, trashed, mimeType)', spaces: 'drive' }); if(DEBUG){console.log(JSON.stringify(res.result.files));} Array.prototype.push.apply(files, res.result.files); if(DEBUG){console.log(files);} files.forEach(function(file) { loadPromises.push(new Promise(async (resolve)=>{ if(file.trashed){ if(DEBUG){console.log('Found trashed file:', file.name, file.id, ', Skipping');} resolve(); return; } if(ex_extensions.includes(file.fileExtension)){ if(DEBUG){console.log('Found excluded file type:', file.name, file.id, ', Skipping');} resolve(); return; } if(DEBUG){console.log('Found file:', file.name, file.id);} const fres = await gapi.client.drive.files.get({ 'fileId': file.id, 'alt': 'media' }); if(file.mimeType.startsWith('text') || file.mimeType.startsWith('multipart') || file.mimeType.startsWith('application')){ console.log(JSON.stringify(file, (k,v) => {if(k != 'body'){return v}return undefined}, 4)); const u8arr = new Uint8Array(fres.body.length * 3); encoder.encodeInto(fres.body, u8arr); FS.writeFile("/GoogleDrive/" + file.name, u8arr); } else{ var bytes = []; for (var i = 0; i < fres.body.length; ++i) { const code = fres.body.charCodeAt(i); bytes.push(code & 0xFF); if(code > 0xFF){ bytes.push(code >> 8); } } FS.writeFile("/GoogleDrive/" + file.name, new Uint8Array(bytes)); } callback_func(Emval.toHandle("/GoogleDrive/" + file.name), Emval.toHandle(file.id)); resolve(); })); }); } Promise.all(loadPromises).then(()=>{if(DEBUG){console.log('Done loading from Google Drive!');}done_callback_func();}); }}) .build(); picker.setVisible(true); }
function __asyncjs__save_to_drive(file_id,fs_path,mime_type) { return Asyncify.handleAsync(async () => { const fileData = FS.readFile(Emval.toValue(fs_path), {encoding: 'utf8'}); const mimetype = Emval.toValue(mime_type); await gapi.client.request({ path: 'https://www.googleapis.com/upload/drive/v3/files/' + Emval.toValue(file_id), method: 'PATCH', body: fileData, params: { uploadType: 'media', mimeType: mimetype, fields: 'id,version,name,mimeType', }, }); }); }
...
function cursor_pos_to_canvas(event) { if (event.type === 'touchmove'){ for (let i of event.changedTouches) { if (GLFW.primaryTouchId === i.identifier) { Browser.setMouseCoords(i.pageX, i.pageY); break; } } } else{ Browser.calculateMouseEvent(event); } dynCall_vidd(GLFW.active.cursorPosFunc, GLFW.active.id, Browser.mouseX, Browser.mouseY) }
function create_button(id,event,callback,pos_x,pos_y,width,height) { let imid = Emval.toValue(id); let btn = document.getElementById(imid); let canvas = document.getElementById('canvas'); if(btn === null){ btn = document.createElement('button'); btn.id = imid; document.body.insertBefore(btn, canvas.nextSibling); } btn.addEventListener('mousemove', cursor_pos_to_canvas, false); btn.addEventListener(Emval.toValue(event), window[Emval.toValue(callback)], false); btn.style.position = 'fixed'; btn.style.left = pos_x + 'px'; btn.style.top = pos_y + 'px'; btn.style.width = width + 'px'; btn.style.height = height + 'px'; btn.style.opacity = 0; }
...
function render_image(id,texture) { var imid = Emval.toValue(id); var img = document.getElementById(imid); if(img === null){ return Emval.toHandle(0); } var canvas = document.getElementById(imid + 'canvas'); if(canvas === null){ canvas = document.createElement('canvas'); canvas.id = imid + 'canvas'; document.body.insertBefore(canvas, document.getElementById('canvas')); canvas.style.position = 'fixed'; canvas.style.top = 100 + 'px'; canvas.style.width = 160 + 'px'; canvas.style.height = 90 + 'px'; } if(img.nodeName == 'IMG'){ canvas.width = img.naturalWidth; canvas.height = img.naturalHeight; } else if(img.nodeName == 'VIDEO'){ canvas.width = img.videoWidth; canvas.height = img.videoHeight; } var ctx = canvas.getContext('2d'); ctx.drawImage(img, 0, 0); var pixels = ctx.getImageData(0, 0, canvas.width, canvas.height); var output = _CreateTexture(texture, Emval.toHandle(pixels.data), canvas.width, canvas.height); ctx = null; pixels = null; return output; }
function get_image_size(id) { var canvas = document.getElementById(imid + 'canvas'); if(canvas === null){ return Emval.toHandle([0, 0]); } return Emval.toHandle([canvas.width, canvas.height]); }
...
function get_preference_value(key) { const output = global_preferences[Emval.toValue(key)]; if(output === '') return Emval.toHandle(''); return Emval.toHandle(output); }
...
function create_audio_element() { } var global_audio_element = null; var global_audio_context = null; var global_audio_blobs = []; var global_audio_completion = []; var global_audio_worker_setup_data = []; var global_audio_compute_kernel = null; if(false){ }
...
function get_audio_context_latency() { if(global_audio_context && ("outputLatency" in global_audio_context)){ return Emval.toHandle(global_audio_context.outputLatency); } return Emval.toHandle(0); }
...
function audio_element_play() { global_audio_element.play(); console.log(global_audio_context.outputLatency); }
function audio_element_pause() { global_audio_element.pause(); }
function get_audio_samples_hybrid(stretch_index,crude_engine,fine_engine) { const crudeEngine = Emval.toValue(crude_engine); const fineEngine = Emval.toValue(fine_engine); const stretchIndex = Emval.toValue(stretch_index); var useCrude = crudeEngine !== ''; var audioWorker = new Worker('plugins/audiostretchworker.js'); audioWorker.postMessage(global_audio_worker_setup_data); audioWorker.postMessage(['Work', useCrude ? crudeEngine : fineEngine, stretchIndex, useCrude]); audioWorker.onmessage = (result) => { if(useCrude){get_audio_samples_hybrid(Emval.toHandle(stretchIndex), Emval.toHandle(''), Emval.toHandle(fineEngine));} global_audio_blobs[result.data[1] - 1] = result.data[0]; global_audio_completion[result.data[1] - 1] = true; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1])); audioWorker.postMessage(['Revive']); audioWorker.onmessage = (result) => { audioWorker.terminate(); if(useCrude){ if(DEBUG){console.log('Resetting corse engine ' + result.data[0]);} } else { if(DEBUG){console.log('Resetting fine engine ' + result.data[0]);} } }; }; }
function get_audio_samples_GPU(stretch_index,kernel_name) { const kernelName = Emval.toValue(kernel_name); const stretchIndex = Emval.toValue(stretch_index); console.time('Audio stretch completed in'); var audioBuff = Module['ShaderBuffer'].Create(); var stretchedBuff = Module['ShaderBuffer'].Create(); const audioLength = global_audio_worker_setup_data[1][0].length; const stretchedLength = Math.ceil(audioLength / (stretchIndex * .1)); if(global_audio_compute_kernel === null) { global_audio_compute_kernel = Module['ComputeShader'].Load('Shaders/ResonateStretchCS.wgsl'); } global_audio_compute_kernel.SetBuffer('audioBuffer', audioBuff); global_audio_compute_kernel.SetBuffer('stretchedBuffer', stretchedBuff); global_audio_compute_kernel.SetInt("audioLength", audioLength); global_audio_compute_kernel.SetInt("stretchedLength", stretchedLength); const stftBins = ( 5 < stretchIndex ? 8192 :( 4 < stretchIndex ? 6140 :( 2 < stretchIndex ? 4096 : 4096))); global_audio_compute_kernel.SetInt("stftBins", stftBins); const stftHop = 1 / ( 5 < stretchIndex ? 3 :( 4 < stretchIndex ? 4.8 :( 2 < stretchIndex ? 5 : 7))); global_audio_compute_kernel.SetFloat("stftHop", stftHop); global_audio_compute_kernel.SetFloat("stretchFactor", 1 / (stretchIndex * 0.1)); const overlap = stftHop * stftBins / (stretchIndex * 0.1); var output = []; new Promise(async(resolveAll)=>{ const wgpuMaxBuffSize = 33500000; for(var ch = 0; ch < global_audio_worker_setup_data[1].length; ch++) { var outputArr = new Float32Array(stretchedLength); var inputLength = stretchedLength < wgpuMaxBuffSize ? audioLength : (wgpuMaxBuffSize * stretchIndex * 0.1); var outputLength = stretchedLength < wgpuMaxBuffSize ? stretchedLength : wgpuMaxBuffSize; var outputStart = 0; var loopInd = 0; for(var start = 0; start < audioLength; start += inputLength){ await new Promise((resolve)=>{ var inputSize = Math.min(inputLength, audioLength - start); var outputSize = Math.min(outputLength, stretchedLength - outputStart); audioBuff.SetData(global_audio_worker_setup_data[1][ch].subarray(start, start + inputSize)); stretchedBuff.SetData(new Float32Array(outputSize)); global_audio_compute_kernel.Dispatch(kernelName, (inputSize - stftBins) / (stftBins * stftHop), 1, 1); stretchedBuff.GetDataAsync((buffer)=>{ outputArr.set(new Float32Array(buffer), outputStart); resolve(); }); }); outputStart += outputLength; loopInd++; } output[ch] = outputArr; } resolveAll(); }).then(()=>{ global_audio_blobs[stretchIndex - 1] = Module.audioDataArrayToBlob(output , global_audio_worker_setup_data[3] ); global_audio_completion[stretchIndex - 1] = true; _jsUpdateAudioBuffer(Emval.toHandle(stretchIndex)); audioBuff.delete(); stretchedBuff.delete(); console.timeEnd('Audio stretch completed in'); }); }
...
function __asyncjs__get_audio_volume_db() { return Asyncify.handleAsync(async () => { return Emval.toHandle(new Promise(async(resolve)=>{ var output = {}; output.peak = 0; output.mean = 0; output.clip_count = 0; if(!global_audio_completion[9]){ resolve(output); return; } global_audio_context.decodeAudioData(await global_audio_blobs[9].arrayBuffer(), (buffer)=>{ for(let ind = 0; ind < buffer.numberOfChannels; ind++){ for(const sample of buffer.getChannelData(ind)){ if(1 < Math.abs(sample)) { output.clip_count++; } else if(output.peak < Math.abs(sample)) { output.peak = Math.abs(sample); } output.mean += Math.abs(sample); } } output.mean /= buffer.getChannelData(0).length * buffer.numberOfChannels; output.clip_count /= buffer.getChannelData(0).length * buffer.numberOfChannels; output.peak = 20 * Math.log10(output.peak ); output.mean = 20 * Math.log10(output.mean ); resolve(output); }); })); }); }
  ...
  var _wgpuShaderModuleGetCompilationInfo = (shaderModuleId, callback, userdata) => {
      ...
      shaderModule["getCompilationInfo"]().then((compilationInfo) => {
        ...
        callUserCallback(() => {
          ...
          ((a1, a2, a3) => dynCall_viii(callback, a1, a2, a3))(0, compilationInfoPtr, userdata);
...
var wasmImports = {
  ...
  /** @export */
  get_audio_context_latency: get_audio_context_latency,
  /** @export */
  ...
  /** @export */
  get_audio_samples_GPU: get_audio_samples_GPU,
  /** @export */
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 5960768;
var ___start_em_js = Module['___start_em_js'] = 5996344;
var ___stop_em_js = Module['___stop_em_js'] = 6033910;
