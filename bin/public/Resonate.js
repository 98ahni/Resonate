...
var ASM_CONSTS = {
  3656757: () => { alert("FileHandler::OpenDocument not implemented!") },  
 3656809: () => { return Emval.toHandle(new Promise((resolve)=>{ FS.syncfs(false, function (err) { if(err){ alert('Unable to sync IndexDB!\n' + err); } resolve(); }); })) },  
 3656966: ($0) => { init_gapi_with_key($0); },  
 3656992: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3657115: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3657217: () => { return Date.now(); },  
 3657238: () => { location.reload() },  
 3657256: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3657319: () => { return global_audio_element.paused ? 1 : 0; },  
 3657363: () => { return global_audio_element.paused ? 1 : 0; },  
 3657407: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 3657457: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3657713: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3657982: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3604412;
var ___start_em_js = Module['___start_em_js'] = 3638816;
var ___stop_em_js = Module['___stop_em_js'] = 3656757;
