...
var ASM_CONSTS = {
  4048569: () => { return Date.now(); },  
 4048590: () => { return Date.now(); },  
 4048611: () => { return Date.now(); },  
 4048632: () => { location.reload() },  
 4048650: () => { location.reload(); },  
 4048669: ($0) => { init_gapi_with_key($0); },  
 4048695: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 4048818: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 4048920: ($0, $1, $2) => { Module.show_loading_screen($0, $1, $2); },  
 4048960: () => { Module.hide_loading_screen(); },  
 4048990: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 4049053: ($0, $1) => { global_audio_element.addEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 4049146: ($0, $1) => { global_audio_element.removeEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 4049242: () => { return global_audio_element.paused ? 1 : 0; },  
 4049286: () => { return global_audio_element.paused ? 1 : 0; },  
 4049330: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 4049380: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 4049636: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 4049905: () => { audio_element_pause(); },  
 4049928: () => { audio_element_play(); },  
 4049950: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3981628;
var ___start_em_js = Module['___start_em_js'] = 4017128;
var ___stop_em_js = Module['___stop_em_js'] = 4048569;
