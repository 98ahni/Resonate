...
var ASM_CONSTS = {
  3655763: () => { alert("FileHandler::OpenDocument not implemented!") },  
 3655815: () => { return Emval.toHandle(new Promise((resolve)=>{ FS.syncfs(false, function (err) { if(err){ alert('Unable to sync IndexDB!\n' + err); } resolve(); }); })) },  
 3655972: ($0) => { init_gapi_with_key($0); },  
 3655998: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3656121: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3656223: () => { return Date.now(); },  
 3656244: () => { location.reload() },  
 3656262: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3656325: () => { return global_audio_element.paused ? 1 : 0; },  
 3656369: () => { return global_audio_element.paused ? 1 : 0; },  
 3656413: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3656669: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3656938: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
function __asyncjs__get_audio_samples_legacy(fs_path) { return Asyncify.handleAsync(async () => { global_audio_worker.postMessage(['Work', 'Legacy', Emval.toValue(stretch_index)]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; global_audio_completion[result.data[1] - 1] = true; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1])); } }); }
function __asyncjs__get_audio_samples_rubberband(fs_path) { return Asyncify.handleAsync(async () => { global_audio_worker.postMessage(['Work', 'RubberBand', Emval.toValue(stretch_index)]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1]] = result.data[0]; global_audio_completion[result.data[1]] = true; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1] + 1)); } }); }
function __asyncjs__get_audio_samples_vexwarp(stretch_index) { return Asyncify.handleAsync(async () => { global_audio_worker.postMessage(['Work', 'VexWarp', Emval.toValue(stretch_index)]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; global_audio_completion[result.data[1] - 1] = true; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1])); } }); }
...
var ___stop_em_js = Module['___stop_em_js'] = 3655763;
