...
var ASM_CONSTS = {
  3383030: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3383153: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3383255: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); }
};
...
function __asyncjs__webgpu_create_device() { return Asyncify.handleAsync(async () => { WebGPU.initManagers(); console.log("Create Start!"); const adapter = await navigator.gpu.requestAdapter(); if(adapter == "undefined") { console.log("Adapter was Undefined!"); } const device = await adapter.requestDevice(); if(device == "undefined") { console.log("Device was Undefined!"); } Module.preinitializedWebGPUDevice = device; console.log("Create End!"); }); }
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3339760;
var ___start_em_js = Module['___start_em_js'] = 3374024;
var ___stop_em_js = Module['___stop_em_js'] = 3383030;
