...
var ASM_CONSTS = {
  3945703: () => { return Date.now(); },  
 3945724: () => { location.reload() },  
 3945742: ($0) => { init_gapi_with_key($0); },  
 3945768: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3945891: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3945993: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3946056: ($0, $1) => { global_audio_element.addEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 3946149: ($0, $1) => { global_audio_element.removeEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 3946245: () => { return global_audio_element.paused ? 1 : 0; },  
 3946289: () => { return global_audio_element.paused ? 1 : 0; },  
 3946333: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 3946383: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3946639: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3946908: () => { audio_element_pause(); },  
 3946931: () => { audio_element_play(); },  
 3946953: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3886260;
var ___start_em_js = Module['___start_em_js'] = 3920832;
var ___stop_em_js = Module['___stop_em_js'] = 3945703;
