...
var ASM_CONSTS = {
  3383686: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3383809: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3383911: () => { return global_audio_element.paused ? 1 : 0; },  
 3383955: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); }
};
...
function __asyncjs__set_audio_playback_file(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3'}); global_audio_blobs.length = 10; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer) => { for(var i = 1; i <= 10; i++){ if(i == 10) global_audio_blobs[i - 1] = Module.audioBufferToBlob(buffer); else { const worker = new Worker('plugins/audiostretchworker.js'); worker.postMessage([global_audio_context.createBuffer(buffer.numberOfChannels, buffer.length * 1 / (i * 0.1), buffer.sampleRate), buffer, i]); worker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; }; } } set_audio_playback_buffer(Emval.toHandle(10)); }); }); }
...
var ___stop_em_js = Module['___stop_em_js'] = 3383686;
