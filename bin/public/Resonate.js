...
var ASM_CONSTS = {
  3383772: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3383895: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3383997: () => { return global_audio_element.paused ? 1 : 0; },  
 3384041: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); }
};
...
function __asyncjs__set_audio_playback_file(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3'}); global_audio_blobs.length = 10; var streatchers = []; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer) => { for(var i = 1; i <= 10; i++){ streatchers.push(new Promise((resolve) => { if(i == 10) global_audio_blobs[i - 1] = Module.audioBufferToBlob(buffer); else if(i == 5) global_audio_blobs[i - 1] = Module.audioBufferToBlob(Module.stretch(global_audio_context, buffer, 2)); else global_audio_blobs[i - 1] = Module.audioBufferToBlob(Module.stretch(global_audio_context, buffer, 1 / (i * 0.1))); console.log('Streched blob nr ' + i); resolve(); })); } set_audio_playback_buffer(Emval.toHandle(10)); }); await Promise.all(streatchers); }); }
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3339404;
var ___start_em_js = Module['___start_em_js'] = 3373672;
var ___stop_em_js = Module['___stop_em_js'] = 3383772;
