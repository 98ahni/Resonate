...
var ASM_CONSTS = {
  4086402: () => { return Date.now(); },  
 4086423: () => { return Date.now(); },  
 4086444: () => { return Date.now(); },  
 4086465: () => { location.reload() },  
 4086483: () => { location.reload(); },  
 4086502: ($0) => { init_gapi_with_key($0); },  
 4086528: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 4086651: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 4086753: ($0, $1, $2) => { Module.show_loading_screen($0, $1, $2); },  
 4086793: () => { Module.hide_loading_screen(); },  
 4086823: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 4086886: ($0, $1) => { global_audio_element.addEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 4086979: ($0, $1) => { global_audio_element.removeEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 4087075: () => { return global_audio_element.paused ? 1 : 0; },  
 4087119: () => { return global_audio_element.paused ? 1 : 0; },  
 4087163: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 4087213: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 4087469: ($0) => { let error_type = $0; let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 4087759: () => { audio_element_pause(); },  
 4087782: () => { audio_element_play(); },  
 4087804: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 4014728;
var ___start_em_js = Module['___start_em_js'] = 4050400;
var ___stop_em_js = Module['___stop_em_js'] = 4086402;
