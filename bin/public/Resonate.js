...
var ASM_CONSTS = {
  3659769: () => { alert("FileHandler::OpenDocument not implemented!") },  
 3659821: () => { return Emval.toHandle(new Promise((resolve)=>{ FS.syncfs(false, function (err) { if(err){ alert('Unable to sync IndexDB!\n' + err); } resolve(); }); })) },  
 3659978: ($0) => { init_gapi_with_key($0); },  
 3660004: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3660127: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3660229: () => { return Date.now(); },  
 3660250: () => { location.reload() },  
 3660268: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3660331: () => { return global_audio_element.paused ? 1 : 0; },  
 3660375: () => { return global_audio_element.paused ? 1 : 0; },  
 3660419: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 3660469: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3660725: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3660994: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
function create_audio_element() { } var global_audio_element = null; var global_audio_context = null; var global_audio_blobs = []; var global_audio_completion = []; var global_audio_worker_corse; var global_audio_worker_fine; var global_audio_worker_setup_data = []; if(false){ }
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3604328;
var ___start_em_js = Module['___start_em_js'] = 3638728;
var ___stop_em_js = Module['___stop_em_js'] = 3659769;
