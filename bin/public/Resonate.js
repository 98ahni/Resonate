...
var ASM_CONSTS = {
  3945989: () => { return Date.now(); },  
 3946010: () => { location.reload() },  
 3946028: ($0) => { init_gapi_with_key($0); },  
 3946054: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3946177: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3946279: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3946342: ($0, $1) => { global_audio_element.addEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 3946435: ($0, $1) => { global_audio_element.removeEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 3946531: () => { return global_audio_element.paused ? 1 : 0; },  
 3946575: () => { return global_audio_element.paused ? 1 : 0; },  
 3946619: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 3946669: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3946925: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3947194: () => { audio_element_pause(); },  
 3947217: () => { audio_element_play(); },  
 3947239: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3886372;
var ___start_em_js = Module['___start_em_js'] = 3920944;
var ___stop_em_js = Module['___stop_em_js'] = 3945989;
