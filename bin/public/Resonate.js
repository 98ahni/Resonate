...
var ASM_CONSTS = {
  3988192: () => { return Date.now(); },  
 3988213: () => { return Date.now(); },  
 3988234: () => { return Date.now(); },  
 3988255: () => { location.reload() },  
 3988273: () => { location.reload(); },  
 3988292: ($0) => { init_gapi_with_key($0); },  
 3988318: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3988441: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3988543: ($0, $1, $2) => { Module.show_loading_screen($0, $1, $2); },  
 3988583: () => { Module.hide_loading_screen(); },  
 3988613: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3988676: ($0, $1) => { global_audio_element.addEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 3988769: ($0, $1) => { global_audio_element.removeEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 3988865: () => { return global_audio_element.paused ? 1 : 0; },  
 3988909: () => { return global_audio_element.paused ? 1 : 0; },  
 3988953: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 3989003: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3989259: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3989528: () => { audio_element_pause(); },  
 3989551: () => { audio_element_play(); },  
 3989573: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
function print_gamepad_state() { const gp = navigator.getGamepads()[0]; let output = ''; gp.buttons.forEach(b => output += '[v:' + b.value + ',p:' + b.pressed + ']'); console.log(output); }
...
var wasmImports = {
  ...
  /** @export */
  print_gamepad_state: print_gamepad_state,
  /** @export */
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3923788;
var ___start_em_js = Module['___start_em_js'] = 3958352;
var ___stop_em_js = Module['___stop_em_js'] = 3988192;
