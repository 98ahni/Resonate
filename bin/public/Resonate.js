...
var ASM_CONSTS = {
  4050785: ($0) => { init_gapi_with_key($0); },  
 4050811: ($0, $1, $2) => { Module.show_loading_screen($0, $1, $2); },  
 4050851: () => { Module.hide_loading_screen(); },  
 4050881: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 4051004: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 4051106: () => { return Date.now(); },  
 4051127: () => { return Date.now(); },  
 4051148: () => { return Date.now(); },  
 4051169: () => { location.reload() },  
 4051187: () => { location.reload(); },  
 4051206: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 4051269: ($0, $1) => { global_audio_element.addEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 4051362: ($0, $1) => { global_audio_element.removeEventListener(Emval.toValue($0), window[Emval.toValue($1)], true); },  
 4051458: () => { return global_audio_element.paused ? 1 : 0; },  
 4051502: () => { return global_audio_element.paused ? 1 : 0; },  
 4051546: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 4051596: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 4051852: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 4052121: () => { audio_element_pause(); },  
 4052144: () => { audio_element_play(); },  
 4052166: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3983296;
var ___start_em_js = Module['___start_em_js'] = 4018784;
var ___stop_em_js = Module['___stop_em_js'] = 4050785;
