...
var ASM_CONSTS = {
  3383072: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3383195: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3383297: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); }
};
...
function create_audio_element() { global_audio_element = new Howl({src:["data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA"]}); } var global_audio_element; if(false){ }
function set_audio_playback_file(fs_path) { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'application/octet-binary'}); const audioURL = URL.createObjectURL(audioBlob); const audio = global_audio_element; audio.src = audioURL; audio.play(); audio.onended = (event) => {console.log("Ended"); _AudioOnEnded();}; audio.onpause = (event) => {console.log("Pause"); _AudioOnPause();}; audio.onplay = (event) => {console.log("Play"); _AudioOnPlay();}; }
function create_audio_playback() { global_audio_element = new Audio(); const audio = global_audio_element; audio.src = "data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA"; audio.load(); audio.play(); }
function get_audio_playback_progress() { const audio = global_audio_element; return Emval.toHandle(audio.seek()); }
function get_audio_duration() { const audio = global_audio_element; return Emval.toHandle(audio.duration()); }
function set_audio_playback_progress(progress) { const audio = global_audio_element; audio.seek(Emval.toValue(progress)); }
function set_audio_playback_speed(play_rate) { const audio = global_audio_element; audio.rate(Emval.toValue(play_rate)); }
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3339808;
var ___start_em_js = Module['___start_em_js'] = 3374072;
var ___stop_em_js = Module['___stop_em_js'] = 3383072;
