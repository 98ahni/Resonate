...
var ASM_CONSTS = {
  3657664: () => { return Date.now(); },  
 3657685: () => { location.reload() },  
 3657703: () => { alert("FileHandler::OpenDocument not implemented!") },  
 3657755: () => { return Emval.toHandle(new Promise((resolve)=>{ FS.syncfs(false, function (err) { if(err){ alert('Unable to sync IndexDB!\n' + err); } resolve(); }); })) },  
 3657912: ($0) => { init_gapi_with_key($0); },  
 3657938: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3658061: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3658163: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3658226: () => { return global_audio_element.paused ? 1 : 0; },  
 3658270: () => { return global_audio_element.paused ? 1 : 0; },  
 3658314: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3658570: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3658839: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
function create_audio_element() { } var global_audio_element = null; var global_audio_context = null; var global_audio_blobs = []; const global_audio_worker = new Worker('plugins/audiostretchworker.js'); if(false){ }
function __asyncjs__set_audio_playback_file(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3'}); global_audio_blobs.length = 10; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer) => { const isSafari = !!window['safari'] && safari !== 'undefined'; global_audio_blobs[9] = Module.audioBufferToBlob(buffer, buffer.sampleRate); set_audio_playback_buffer(Emval.toHandle(10)); var audioDatas = []; audioDatas.length = buffer.numberOfChannels; for(var i = 0; i < buffer.numberOfChannels; i++){ audioDatas[i] = buffer.getChannelData(i); } global_audio_worker.postMessage(['Legacy', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; }; }); }); }
...
function create_audio_playback_with_worklet() { global_audio_element = new Audio(); const audio = global_audio_element; const AudioContext = window.AudioContext || window.webkitAudioContext; global_audio_context = new AudioContext(); global_audio_context.audioWorklet.addModule("plugins/rubberbandworklet.js").then(()=>{ const track = global_audio_context.createMediaElementSource(global_audio_element); const rubberband = new AudioWorkletNode(global_audio_context, "rubberband-processor"); track.connect(rubberband); rubberband.connect(global_audio_context.destination); }); audio.src = "data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA"; if(audio.hasAttribute("webkitPreservesPitch")) { audio.webkitPreservesPitch = true; } else { audio.preservesPitch = true; } console.log(global_audio_context.state); global_audio_context.resume(); audio.play().then(()=>{ audio.pause(); }); window.onpagehide = (e) => { global_audio_context.close(); }; }
...
function __asyncjs__get_audio_samples_rubberband(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3' }); global_audio_blobs.length = 10; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer)=>{ const isSafari = !!window['safari'] && safari !== 'undefined'; global_audio_blobs[9] = Module.audioBufferToBlob(buffer, buffer.sampleRate); set_audio_playback_buffer(Emval.toHandle(10)); var audioDatas = []; audioDatas.length = buffer.numberOfChannels; for(var i = 0; i < buffer.numberOfChannels; i++){ audioDatas[i] = buffer.getChannelData(i); } global_audio_worker.postMessage(['RubberBand', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1]] = result.data[0]; } }); }); }
function __asyncjs__get_audio_samples_vexwarp(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3' }); global_audio_blobs.length = 10; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer)=>{ const isSafari = !!window['safari'] && safari !== 'undefined'; global_audio_blobs[9] = Module.audioBufferToBlob(buffer, buffer.sampleRate); set_audio_playback_buffer(Emval.toHandle(10)); var audioDatas = []; audioDatas.length = buffer.numberOfChannels; for(var i = 0; i < buffer.numberOfChannels; i++){ audioDatas[i] = buffer.getChannelData(i); } global_audio_worker.postMessage(['VexWarp', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; } }); }); }
function __asyncjs__get_audio_samples_paulstretch(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3' }); global_audio_blobs.length = 10; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer)=>{ const isSafari = !!window['safari'] && safari !== 'undefined'; global_audio_blobs[9] = Module.audioBufferToBlob(buffer, buffer.sampleRate); set_audio_playback_buffer(Emval.toHandle(10)); var audioDatas = []; audioDatas.length = buffer.numberOfChannels; for(var i = 0; i < buffer.numberOfChannels; i++){ audioDatas[i] = buffer.getChannelData(i); } global_audio_worker.postMessage(['PaulStretch', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; } }); }); }
function __asyncjs__get_audio_samples_hybrid(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3' }); global_audio_blobs.length = 10; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer)=>{ const isSafari = !!window['safari'] && safari !== 'undefined'; global_audio_blobs[9] = Module.audioBufferToBlob(buffer, buffer.sampleRate); set_audio_playback_buffer(Emval.toHandle(10)); var audioDatas = []; audioDatas.length = buffer.numberOfChannels; for(var i = 0; i < buffer.numberOfChannels; i++){ audioDatas[i] = buffer.getChannelData(i); } global_audio_worker.postMessage(['LegacyHybrid', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; global_audio_worker.postMessage(['RubberBand', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); global_audio_worker.onmessage = (result) => { global_audio_blobs[result.data[1]] = result.data[0]; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1] + 1)); } } }); }); }
...
var wasmImports = {
  ...
  /** @export */
  __asyncjs__get_audio_samples_hybrid: __asyncjs__get_audio_samples_hybrid,
  /** @export */
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3601552;
var ___start_em_js = Module['___start_em_js'] = 3635960;
var ___stop_em_js = Module['___stop_em_js'] = 3657664;
