...
var ASM_CONSTS = {
  3383208: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3383331: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3383433: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); }
};
...
function create_audio_element() { global_audio_element = new Howl({src:["data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA"]}); } var global_audio_element = null; if(false){ }
function set_audio_playback_file(fs_path) { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'application/octet-binary'}); const audioURL = URL.createObjectURL(audioBlob); global_audio_element = new Audio(); const audio = global_audio_element; try { audio.srcObject = audioBlob; } catch (e) {} audio.src = audioURL; alert('made it'); audio.play(); audio.onended = (event) => {console.log("Ended"); _AudioOnEnded();}; audio.onpause = (event) => {console.log("Pause"); _AudioOnPause();}; audio.onplay = (event) => {console.log("Play"); _AudioOnPlay();}; }
function create_audio_playback() { global_audio_element = new Audio(); const audio = global_audio_element; audio.src = "data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA"; audio.play(); }
...
var ___stop_em_js = Module['___stop_em_js'] = 3383208;
