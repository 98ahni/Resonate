...
var ASM_CONSTS = {
  3658165: () => { alert("FileHandler::OpenDocument not implemented!") },  
 3658217: () => { return Emval.toHandle(new Promise((resolve)=>{ FS.syncfs(false, function (err) { if(err){ alert('Unable to sync IndexDB!\n' + err); } resolve(); }); })) },  
 3658374: ($0) => { init_gapi_with_key($0); },  
 3658400: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3658523: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3658625: () => { return Date.now(); },  
 3658646: () => { location.reload() },  
 3658664: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3658727: () => { return global_audio_element.paused ? 1 : 0; },  
 3658771: () => { return global_audio_element.paused ? 1 : 0; },  
 3658815: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 3658865: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3659121: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3659390: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
function request_client_token(prompt,token_callback) { global_client_token.callback = (token_data) =>{Module[token_callback](Emval.toHandle(Date.now() + token_data.expires_in));}; global_client_token.requestAccessToken({prompt: Emval.toValue(prompt)}); }
...
function create_audio_element() { } var global_audio_element = null; var global_audio_context = null; var global_audio_blobs = []; var global_audio_completion = []; var global_audio_worker_corse = new Worker('plugins/audiostretchworker.js'); var global_audio_worker_fine = new Worker('plugins/audiostretchworker.js'); if(false){ }
...
function get_audio_samples_hybrid(stretch_index,crude_engine,fine_engine) { const crudeEngine = Emval.toValue(crude_engine); const fineEngine = Emval.toValue(fine_engine); const stretchIndex = Emval.toValue(stretch_index); var useCrude = crudeEngine !== ''; var audioWorker = useCrude ? global_audio_worker_corse : global_audio_worker_fine; audioWorker.postMessage(['Work', useCrude ? crudeEngine : fineEngine, stretchIndex, useCrude]); audioWorker.onmessage = (result) => { if(useCrude){get_audio_samples_hybrid(Emval.toHandle(stretchIndex), Emval.toHandle(''), Emval.toHandle(fineEngine));} global_audio_blobs[result.data[1] - 1] = result.data[0]; global_audio_completion[result.data[1] - 1] = true; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1])); audioWorker.postMessage(['Revive']); audioWorker.onmessage = (result) => { audioWorker.terminate(); if(useCrude){ global_audio_worker_corse = new Worker('plugins/audiostretchworker.js'); console.log('Resetting corse'); } else { global_audio_worker_fine = new Worker('plugins/audiostretchworker.js'); console.log('Resetting fine'); } result.data[0] = 'Setup'; (useCrude ? global_audio_worker_corse : global_audio_worker_fine).postMessage(result.data); }; } }
function get_audio_samples_legacy(stretch_index,use_crude) { var useCrude = Emval.toValue(use_crude); var audioWorker = useCrude ? global_audio_worker_corse : global_audio_worker_fine; audioWorker.postMessage(['Work', 'Legacy', Emval.toValue(stretch_index), useCrude]); audioWorker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; global_audio_completion[result.data[1] - 1] = true; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1])); audioWorker.postMessage(['Revive']); audioWorker.onmessage = (result) => { audioWorker.terminate(); if(useCrude){ global_audio_worker_corse = new Worker('plugins/audiostretchworker.js'); console.log('Resetting corse'); } else { global_audio_worker_fine = new Worker('plugins/audiostretchworker.js'); console.log('Resetting fine'); } result.data[0] = 'Setup'; (useCrude ? global_audio_worker_corse : global_audio_worker_fine).postMessage(result.data); }; } }
function get_audio_samples_rubberband(stretch_index,use_crude) { var useCrude = Emval.toValue(use_crude); var audioWorker = useCrude ? global_audio_worker_corse : global_audio_worker_fine; audioWorker.postMessage(['Work', 'RubberBand', Emval.toValue(stretch_index), useCrude]); audioWorker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; global_audio_completion[result.data[1] - 1] = true; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1])); audioWorker.postMessage(['Revive']); audioWorker.onmessage = (result) => { audioWorker.terminate(); if(useCrude){ global_audio_worker_corse = new Worker('plugins/audiostretchworker.js'); console.log('Resetting corse'); } else { global_audio_worker_fine = new Worker('plugins/audiostretchworker.js'); console.log('Resetting fine'); } result.data[0] = 'Setup'; (useCrude ? global_audio_worker_corse : global_audio_worker_fine).postMessage(result.data); }; } }
function get_audio_samples_vexwarp(stretch_index,use_crude) { var useCrude = Emval.toValue(use_crude); var audioWorker = useCrude ? global_audio_worker_corse : global_audio_worker_fine; audioWorker.postMessage(['Work', 'VexWarp', Emval.toValue(stretch_index), useCrude]); audioWorker.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; global_audio_completion[result.data[1] - 1] = true; _jsUpdateAudioBuffer(Emval.toHandle(result.data[1])); audioWorker.postMessage(['Revive']); audioWorker.onmessage = (result) => { audioWorker.terminate(); if(useCrude){ global_audio_worker_corse = new Worker('plugins/audiostretchworker.js'); console.log('Resetting corse'); } else { global_audio_worker_fine = new Worker('plugins/audiostretchworker.js'); console.log('Resetting fine'); } result.data[0] = 'Setup'; (useCrude ? global_audio_worker_corse : global_audio_worker_fine).postMessage(result.data); }; } }
function __asyncjs__get_audio_samples_paulstretch(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3' }); global_audio_blobs.length = 10; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer)=>{ const isSafari = !!window['safari'] && safari !== 'undefined'; global_audio_blobs[9] = Module.audioBufferToBlob(buffer, buffer.sampleRate); set_audio_playback_buffer(Emval.toHandle(10)); var audioDatas = []; audioDatas.length = buffer.numberOfChannels; for(var i = 0; i < buffer.numberOfChannels; i++){ audioDatas[i] = buffer.getChannelData(i); } global_audio_worker_corse.postMessage(['PaulStretch', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); global_audio_worker_corse.onmessage = (result) => { global_audio_blobs[result.data[1] - 1] = result.data[0]; } }); }); }
function __asyncjs__get_audio_samples_setup(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3' }); global_audio_blobs.length = 10; global_audio_completion = [false, false, false, false, false, false, false, false, false, false]; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer)=>{ const isSafari = !!window['safari'] && safari !== 'undefined'; global_audio_blobs[9] = Module.audioBufferToBlob(buffer, buffer.sampleRate); global_audio_completion[9] = true; set_audio_playback_buffer(Emval.toHandle(10)); var audioDatas = []; audioDatas.length = buffer.numberOfChannels; for(var i = 0; i < buffer.numberOfChannels; i++){ audioDatas[i] = buffer.getChannelData(i); } global_audio_worker_corse.postMessage(['Setup', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); global_audio_worker_fine.postMessage(['Setup', audioDatas, audioDatas[0].length, buffer.sampleRate, isSafari]); }); }); }
function __asyncjs__get_audio_samples_no_stretch(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3' }); global_audio_blobs.length = 10; global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer)=>{ global_audio_blobs[9] = Module.audioBufferToBlob(buffer, buffer.sampleRate); set_audio_playback_buffer(Emval.toHandle(10)); }); }); }
...
function resize_canvas() { document.getElementById('canvas').width = window.innerWidth; document.getElementById('canvas').height = window.innerHeight; }
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3601480;
var ___start_em_js = Module['___start_em_js'] = 3635880;
var ___stop_em_js = Module['___stop_em_js'] = 3658165;
