...
var ASM_CONSTS = {
  3383731: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3383854: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3383956: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); }
};
...
function create_audio_element() { } var global_audio_context = null; var global_audio_element = null; var global_audio_blobs = []; if(false){ }
function __asyncjs__set_audio_playback_file(fs_path) { return Asyncify.handleAsync(async () => { const audioData = FS.readFile(Emval.toValue(fs_path)); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3'}); global_audio_blobs.length = 10; const blobBuffer = await audioBlob.arrayBuffer(); global_audio_context.decodeAudioData(blobBuffer, (buffer) => { for(var i = 1; i <= 10; i++){ global_audio_blobs[i - 1] = Module.audioBufferToBlob(Module.stretch(global_audio_context, buffer, 1 / (i * 0.1))); } set_audio_playback_buffer(Emval.toHandle(10)); }); }); }
...
function create_audio_playback() { global_audio_element = new Audio(); const audio = global_audio_element; const AudioContext = window.AudioContext || window.webkitAudioContext; const global_audio_context = new AudioContext(); const track = global_audio_context.createMediaElementSource(global_audio_element); track.connect(global_audio_context.destination); audio.src = "data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA"; if(audio.hasAttribute("webkitPreservesPitch")) { audio.webkitPreservesPitch = true; } else { audio.preservesPitch = true; } console.log(global_audio_context.state); global_audio_context.resume().then(()=>{ audio.pause(); }); audio.play(); }
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3339680;
var ___start_em_js = Module['___start_em_js'] = 3373944;
var ___stop_em_js = Module['___stop_em_js'] = 3383731;
