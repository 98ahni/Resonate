...
var ASM_CONSTS = {
  3673251: () => { return Date.now(); },  
 3673272: () => { location.reload() },  
 3673290: () => { alert("FileHandler::OpenDocument not implemented!") },  
 3673342: () => { return Emval.toHandle(new Promise((resolve)=>{ FS.syncfs(false, function (err) { if(err){ alert('Unable to sync IndexDB!\n' + err); } resolve(); }); })) },  
 3673499: ($0) => { init_gapi_with_key($0); },  
 3673525: () => { if(document.getElementById('temp-text-input')) { document.getElementById('temp-text-input').focus({preventScroll: true});} },  
 3673648: () => { if(document.getElementById('temp-file-input')) { document.getElementById('temp-file-input').click();} },  
 3673750: () => { if(global_audio_context !== null)global_audio_context.close(); },  
 3673813: () => { return global_audio_element.paused ? 1 : 0; },  
 3673857: () => { return global_audio_element.paused ? 1 : 0; },  
 3673901: ($0) => { return global_audio_completion[($0) - 1] ? 1 : 0; },  
 3673951: ($0) => { if(!document.querySelector("link[rel='icon']")) { let link = document.createElement('link'); link.rel = 'icon'; link.type = 'image/png'; document.head.appendChild(link); } document.querySelector("link[rel='icon']").href = "icons/" + Emval.toValue($0); },  
 3674207: () => { let errString = 'Undefined'; if(error_type === 1) errString = 'Validation'; else if(error_type === 2) errString = 'Out of memory'; else if(error_type === 4) errString = 'Unknown'; else if(error_type === 5) errString = 'Device lost'; alert('WebGPU Error ' + errString); },  
 3674476: () => { const dbname = '/local'; var req = indexedDB.deleteDatabase(dbname); req.onsuccess = function() { console.log('Deleted IndexedDB /local!'); location.reload();}; req.onerror = function() { console.error('Failed to delete IndexedDB /local!');}; req.onblocked = function() { console.error('Failed to delete IndexedDB /local, DB was blocked!');}; }
};
...
function __asyncjs__setup_latency_metronome() { return Asyncify.handleAsync(async () => { if(global_metronome_source !== null) {return;} const audioData = FS.readFile('/Sound/Metronome.mp3'); const audioBlob = new Blob([audioData.buffer], {type: 'audio/mp3' }); global_audio_context.decodeAudioData(await audioBlob.arrayBuffer(), (buffer)=>{ global_metronome_source = global_audio_context.createBufferSource(); global_metronome_source.buffer = buffer; global_metronome_source.loop = true; global_metronome_source.connect( global_audio_context.destination); }); }); }
function play_latency_metronome() { global_metronome_source.start(); } var global_metronome_source = null;
function stop_latency_metronome() { global_metronome_source.stop(); }
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 3620208;
var ___start_em_js = Module['___start_em_js'] = 3654624;
var ___stop_em_js = Module['___stop_em_js'] = 3673251;
